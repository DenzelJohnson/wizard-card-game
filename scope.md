...
- Core UI intent contract -> `HomeScreen` receives save/warning flags and a difficulty-aware start callback, exposes enabled Easy and Medium modes, and retains disabled Hard Beta; `GameTable` receives `GameState`, readonly controller-provided legal actions, one `GameAction` callback, and an optional storage-warning flag rendered inside its main landmark, derives control availability only by exact action matching, sorts the human hand for display as Spades/Hearts/Clubs/Diamonds descending then Wizards then Jesters without mutating engine state, leaves the normal top-left table grid cell empty, enlarges the dealer marker, removes the visible opponent card-count badge, and places enlarged opponent bid/trick counters on the felt outside their seat panels; `TrickArea` renders the actual non-interactive face-up card at the dealer-facing edge of the central felt with no visible “Trump: suit” copy, adds only a visual suit medallion when a revealed Wizard has chosen trump, and keeps equivalent card/trump context in its accessible name; `src/App.tsx` consumes UI intents and routes them to `useWizardGame`
...
- Visual styling contract -> `src/components/` produces semantic class names and card data hooks; `TrickArea` always renders four player-owned trick slots in a cross (Ember left, Rowan top, Mira right, human bottom) and fills them by `PlayedCard.playerId`, while `src/styles/global.css`, `table.css`, `cards.css`, and `dialogs.css` consume those hooks in cascade order from `src/main.tsx`; fallback dialogs expose `data-fallback="true"` for viewport-centering when native modal APIs are unavailable; accessibility tests protect landmark, heading, focus, accessible-name/description, hidden-information, card ownership, card-face, result-surface, and table/dialog semantics without coupling to pixel values
