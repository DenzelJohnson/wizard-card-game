...
- Round-score row presentation -> `RoundSummary` groups a player's name and parenthesized signed round change at the left edge, while its cumulative `Total` remains aligned to the far right; `src/styles/table.css` supplies the row layout and component tests retain the displayed score content
