...
trick-taking game. It supports local play against three computer opponents and private online rooms
for two to four people, with computers filling unclaimed seats. Easy mode uses random legal computer
decisions and Medium uses deterministic heuristics; Hard remains a disabled beta option. The browser
application is published through GitHub Pages, while Supabase supplies anonymous player identity,
room membership, and real-time synchronization for online matches.
...
- Persistence: versioned browser `localStorage` for local matches; Supabase Postgres and Realtime for online rooms
- Runtime integration: `@supabase/supabase-js` with the project URL and publishable key only; no service-role key is ever shipped to browsers
...
| `src/app/` | Application orchestration for local and online games. `useWizardGame` owns local persistence; the online controller owns a member's room connection, host-authoritative transitions, computer turns, and recovery UI. | Engine, AI, storage, Supabase room/membership records, secure/fallback seed sources, motion/timing preferences, UI intents | Local state/storage requests; validated room actions and host-authored room-state revisions |
| `src/multiplayer/` | Supabase client boundary, typed room/membership contracts, anonymous identity, private-room create/join/leave operations, real-time subscriptions, sealed online-bid checks, and host action messages. | Public project URL/key, Supabase Auth, Postgres, Realtime, game contracts | Anonymous session, `wizard_rooms`, `wizard_room_members`, room state revisions, sealed bid intents, and private Realtime messages |
...
| `tests/` and `e2e/` | Unit, integration, and Playwright browser verification, including full-match, resume, dialog, motion, responsive-layout, production-asset, live multiplayer, and live Supabase RLS/RPC security checks | Public engine/UI contracts, publishable Supabase configuration, plus non-sensitive phase/round/active-player/legal-card DOM hooks | Temporary test rooms, reports, and uncommitted visual-review screenshots only |
...
Supabase project `wizard-card-game` (`efibspovzdykavswvdwm`) owns the following explicitly exposed,
RLS-protected Postgres tables. The Data API default exposure for newly created tables is disabled.

| Table | Key fields | Producers | Consumers | Access model |
|-------|------------|-----------|-----------|--------------|
| `public.wizard_rooms` | `id`, unique `room_code`, `host_user_id`, `status`, `difficulty`, `human_seat_count`, monotonic `revision`, timestamps | Secure room RPCs | Online controller, joining lobby UI, members' row-change subscriptions | A member may read its room; direct browser writes are denied; RLS is enabled |
| `public.wizard_room_members` | `room_id`, `user_id`, unique `seat_id` per room, `display_name`, timestamps | Host creates its membership; joining players insert their own membership into a lobby | Online controller, lobby presence, private Realtime authorization policies | A member may read room members; a user may create/remove only their own lobby membership; RLS is enabled |
| `public.wizard_game_states` | `room_id`, serialized authoritative `game_state`, `revision`, timestamp | Host-only optimistic commit RPC | Reconnecting room host only | Only the room host may read; direct browser writes are denied; RLS is enabled |
| `public.wizard_player_states` | `room_id`, `user_id`, redacted serialized `game_state`, `revision`, timestamp | Host-only optimistic commit RPC | The matching room member and its Postgres Changes subscription | A member may read only its own state; direct browser writes are denied; RLS is enabled |
| `public.wizard_actions` | identity, `room_id`, authenticated `user_id`, expected `revision`, typed action JSON, timestamp | Identity-checking submit-action RPC | Room host and its row-change subscription; member-scoped bid-lock RPC | Only the host may read action values; members can ask only whether their own bid is locked; the RPC proves the sender owns the action's seat, makes bid submissions final, and committed actions are removed atomically |

`realtime.messages` policies authorize private topic `room:<room UUID>` only when the authenticated
anonymous user has a corresponding `wizard_room_members` row. Public Realtime channel access is disabled.
...
| Supabase | Anonymous Auth, RLS-protected Postgres rooms/members, and private Realtime room actions | User's free Supabase organization; browser uses only the project URL and publishable API key | `src/multiplayer/` and online UI/controller |

No paid runtime service, analytics, or private server credential is in scope.
...
- `GameState` and action contracts -> produced by `src/game/`; consumed by `src/app/`, `src/ai/`, `src/storage/`, `src/components/`, `src/multiplayer/`, and tests; online matches preserve the four engine seat IDs while their members map those seats to devices and computer ownership
...
- Visual styling contract -> `src/components/` produces semantic class names and card data hooks; `TrickArea` always renders four player-owned trick slots in a cross (Ember left, Rowan top, Mira right, human bottom) and fills them by `PlayedCard.playerId`; the played-card/empty-slot dimensions and their cross container are approximately twice their original dimensions without changing the separate face-up trump card, which reserves clear space from every seat's bid/trick counters during bidding, while `src/styles/global.css`, `table.css`, `cards.css`, and `dialogs.css` consume those hooks in cascade order from `src/main.tsx`; active game tables use a responsive composition scale so the full board and complete human hand fit inside supported viewports without document or hand scrolling while preserving the relative component proportions; cards inside `.human-hand` use a hand-only size override set 20% below their former enlarged size, with proportionally reduced overlap; bidding keeps disabled hand cards at full color, while in card-play phases unavailable cards retain their native disabled state and receive a clearly greyed-out treatment so an illegal follow-suit move is visibly distinct from legal cards; fallback dialogs expose `data-fallback="true"` for viewport-centering when native modal APIs are unavailable; accessibility tests protect landmark, heading, focus, accessible-name/description, hidden-information, card ownership, card-face, result-surface, table/dialog semantics without coupling to pixel values
- Hand trump-card styling -> `src/styles/table.css` renders human-hand cards matching the resolved trump suit with a thick purple outline and glow; `PlayingCard` supplies the existing `.playing-card--trump` hook and browser tests verify the computed presentation
- Package scripts and lockfile -> produced by project configuration; consumed by local verification and GitHub Actions
- Playwright configuration -> `PLAYWRIGHT_BASE_URL` selects an already-running server without starting one; when absent, Playwright starts Vite on a documented strict loopback port with strict-port binding; desktop and mobile Chromium projects consume the same public UI in one worker in every environment, including a complete-game-table viewport-fit assertion with no document or hand scrolling, and every runner process uses its own PID-scoped ignored output directory so concurrent invocations cannot delete each other's trace artifacts; isolated contexts clear `localStorage`, capture failure diagnostics, and write manual-review screenshots under ignored test output
...
- Online room contract -> secure RPCs create/join/leave rooms, automatically assign the next open human seat, strictly validate member action type/shape/phase/turn ownership, deduplicate queued actions per player/revision, and atomically commit state; in online bidding, each human locks one final bid at the current revision regardless of the legacy engine's active-seat order, computer bids are generated without observing those hidden values, and the host reduces all four bids in canonical player order then publishes them in one commit. The member-scoped bid-lock RPC reveals only the caller's lock status, never a bid value or another member's status. Every online client reads its member-authorized room/members, while guests subscribe to their own redacted `wizard_player_states` row scoped to both the active room and user and the host subscribes to authenticated `wizard_actions`; stale subscription generations are ignored and bounded reconciliation recovers missed guest/host events; only the room host reduces legal actions, runs computers/timers, and commits the full `wizard_game_states` value with optimistic revision locking. Full hands are visible only to the host; each guest row clears every other hand. Room creation permits one active hosted room per identity and removes rooms inactive for 24 hours. This v1 model intentionally requires the host browser to remain connected while a game is running.
- Online identity/config contract -> `src/multiplayer/supabase.ts` consumes a public Supabase URL and publishable key. Anonymous Auth is enabled only for this project. RLS policies and private Realtime authorization enforce room membership; neither service-role nor database passwords may appear in source, built assets, workflow logs, or browser storage.
