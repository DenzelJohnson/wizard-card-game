...
| `public.wizard_actions` | identity, `room_id`, authenticated `user_id`, expected `revision`, typed action JSON, timestamp | Identity-checking submit-action RPC | Room host and its row-change subscription | Only the host may read; the RPC proves the sender owns the action's seat; committed actions are removed atomically |
...
- Online room contract -> secure RPCs create/join/leave rooms, automatically assign the next open human seat, strictly validate member action type/shape/phase/turn ownership, deduplicate queued actions per player/revision, and atomically commit state; every online client reads its member-authorized room/members, while guests subscribe to their own redacted `wizard_player_states` row scoped to both the active room and user and the host subscribes to authenticated `wizard_actions`; stale subscription generations are ignored and bounded reconciliation recovers missed guest/host events; only the room host reduces legal actions, runs computers/timers, and commits the full `wizard_game_states` value with optimistic revision locking. Full hands are visible only to the host; each guest row clears every other hand. Room creation permits one active hosted room per identity and removes rooms inactive for 24 hours. This v1 model intentionally requires the host browser to remain connected while a game is running.
