...
- Result/menu UI contract -> `GameTable` receives explicit round-continue, restart, and home callbacks from `src/App.tsx`; unfinished restart/home requests are confirmed in `GameMenu`, while completed matches may navigate immediately; `RoundSummary` and `ScoreSheet` read stored `roundScores`, and `MatchResult` uses engine `matchWinners` with stable seat-order tie breaking for its displayed standings
- Sound preference contract -> `src/audio/sounds.ts` alone reads and writes exact boolean text under `wizard-card-game/sound-enabled`; `GameMenu` exposes the opt-in toggle, and `GameTable` emits card/trick/round presentation cues only after observed state transitions; the preference is independent of `GameState` and `wizard-card-game/save-v1`, and storage or Web Audio failures never affect gameplay
