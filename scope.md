...
| `src/app/` | Application orchestration and phase progression through `useWizardGame`, whose controller exposes home/game navigation, resume status, storage warnings, engine legal actions, human dispatch, explicit round acknowledgement, and abandon controls | Engine, AI, storage, motion/timing preferences, UI intents | State updates and persistence requests |
...
- Wizard controller contract -> `src/app/useWizardGame.ts` is the sole live-state owner and exposes screen/state/save-warning/legal-action values plus start, continue, human dispatch, explicit round acknowledgement, and abandon commands; future `src/App.tsx` and `src/components/` consume this controller while the hook alone schedules round setup, Easy computer turns, and trick acknowledgement
