...
create or replace function public.has_wizard_bid_lock(
  p_room_id uuid,
...
begin
  ...
  if not exists (
    ...
    raise exception 'Room membership is invalid';
  end if;

  return exists (
    select 1
    from public.wizard_actions action
...
revoke all on function public.has_wizard_bid_lock(uuid, bigint) from public, anon;
grant execute on function public.has_wizard_bid_lock(uuid, bigint) to authenticated;
